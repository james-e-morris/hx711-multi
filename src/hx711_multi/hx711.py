...
class HX711:
    ...
    def __init__(self,
        ...
        self.logger: Logger = getLogger('hx711-multi')
    ...
    def _prepare_to_read(self):
        ...
        if ready:
            self.logger.debug(f'checked sensor readiness, completed after {i+1} iterations')
        else:
            self.logger.warn(f'checked sensor readiness, not ready after {i+1} iterations')              
        return ready
    ...
    def _pulse_sck_high(self):
        ...
        if pulse_end - pulse_start >= 0.00006:  # check if the hx 711 did not turn off...
            ...
            self.logger.warn(f'sck pulse lasted for longer than 60us\nTime elapsed: {pulse_end - pulse_start}')
            return False
        return True
    ...
    def read_raw(self, readings_to_average: int = 10):
        ...
        self.logger.info(f'Finished read operation. Load cell results:\n{all_load_cell_vars}')
        ...
        if not load_cell_measurements or all(x is None for x in load_cell_measurements):
            self.logger.warning(f'All load cell measurements failed. '
    ...
    def zero(self, readings_to_average: int = 30):
        ...
        for load_cell in self._load_cells:
            if load_cell._ready:
                self.logger.debug(f'zeroing with {len(load_cell._reads_filtered)} datapoints')
...
class LoadCell:
    ...
    def _finish_raw_read(self):
        ...
        self.logger.debug(f'Binary value: {bin(self._current_raw_read)} -> Signed: {self._current_signed_value}')
    ...
    def convert_to_signed_value(self, raw_value):
        ...
        if raw_value in [0x800000, 0x7FFFFF, 0xFFFFFF]:
            self.logger.debug('Invalid raw value detected: {}'.format(hex(raw_value)))
    ...
    def _calculate_measurement(self):
        ...
        if self._read_stdev > self._max_stdev:
            ...
            self._ready = False
            self.logger.warn(f'load cell (dout {self._dout_pin}) not ready, stdev from median was over {self._max_stdev}: {self._read_stdev}')
